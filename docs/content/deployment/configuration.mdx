...
On Kubernetes, set the logging properties under `configuration.log` in your Helm values rather than editing `deployment.yaml`. See [Logging](../deployment-paths/kubernetes#logging).
...
| `database.config.sqlite.options` | `_pragma=busy_timeout(5000)&_pragma=journal_mode(WAL)&_pragma=foreign_keys(1)` | SQLite connection options |
...
| `database.runtime_transient.sqlite.options` | `_pragma=busy_timeout(5000)&_pragma=journal_mode(WAL)&_pragma=foreign_keys(1)` | SQLite connection options |
...
| `database.entity.sqlite.options` | `_pragma=busy_timeout(5000)&_pragma=journal_mode(WAL)&_pragma=foreign_keys(1)` | SQLite connection options |
...
| `database.runtime_persistent.sqlite.options` | `_pragma=busy_timeout(5000)&_pragma=journal_mode(WAL)&_pragma=foreign_keys(1)` | SQLite connection options |
...
| Setting | Default | Description |
|---------|---------|-------------|
| `oauth.refresh_token.renew_on_grant` | `true` | If `true`, issues a new refresh token on each access token grant. Set to `false` to reuse the existing refresh token until it expires |
...
| `oauth.dcr.registration_access_token_validity_period` | `7776000` | Lifetime in seconds of the registration access token a client uses to manage its own registration (RFC 7592) |
...
| `oauth.allowed_response_types` | `["code"]` | OAuth response types allowed during client registration |
| `oauth.allowed_grant_types` | `["client_credentials", "authorization_code", "refresh_token", "urn:ietf:params:oauth:grant-type:token-exchange", "urn:openid:params:grant-type:ciba", "urn:ietf:params:oauth:grant-type:jwt-bearer"]` | OAuth grant types allowed during client registration |
...
| `oauth.send_server_errors_to_client` | `false` | If `true`, an authentication flow failure that maps to the OAuth `server_error` code is reported to the client, as RFC 6749 section 4.1.2.1 requires. If `false`, the authorization code flow shows the error page instead of redirecting to the client, and CIBA leaves the request pending so the polling client times out. Denials (`access_denied`) are always reported to the client and are not affected by this setting. |
...
Templates (email and other templated content) are now supported as a declarative resource. See the templates guide for schema, examples, and the configured directory location: [Template declarative resources](../guides/declarative-configurations/templates.mdx).
...
On Kubernetes, set these properties under `configuration.email.smtp` in your Helm values rather than editing `deployment.yaml`. The chart also sources the SMTP password from a Kubernetes Secret. See [Email](../deployment-paths/kubernetes#email).
...
| Setting | Default | Description |
|---------|---------|-------------|
| `authn_provider.rest.enabled` | `false` | Set to `true` to register the REST authentication provider. When `false`, the rest of the `rest.*` settings are ignored. |
| `authn_provider.rest.credential_types` | `[]` | Credential keys routed to the REST provider instead of `default`. Valid keys: `provisionedEntityID`, `passkey`, `otp`, `federated`, `magiclink`, `openid4vp`, `password`, `clientSecret`, `flowSecret`. |
...
Controls server-wide default flows and context expiry times for each flow type. These settings are stored in the server-config `flow` section, not in `deployment.yaml`.

Each flow type has two fields:
...
1. Application-level override (the flow ID pinned on the application or agent)
2. Organization unit default (the flow ID configured on the owning OU)
3. Server default from this section (the handle is looked up to find the flow ID)

If none of the layers supplies a value, registration, recovery, user-onboarding, and sign-out are left not configured (the feature is treated as disabled for that client). Authentication has no such fallback: a missing auth flow causes startup validation to fail.

Configure flow defaults in either of these ways:

- **Declarative** - create `config/resources/server_configs/flow.yaml`, then restart:
  ```yaml
  name: flow
  value:
    authFlow:
    ...
    signOutFlow:
      expirySeconds: 1800
  ```
- **Runtime** - send the new value to `PUT /server-config/flow`; it is applied immediately and merged with any declarative file:
  ```http
